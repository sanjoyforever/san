...
s1.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,jpg"
s1.linkInternalFilters="javascript:,etihadairways.com,etihad.com"
...
/* To lower case */
if (s1.pageName) s1.pageName = s1.pageName.toLowerCase();
if (s1.channel) s1.channel = s1.channel.toLowerCase();
if (s1.prop7) s1.prop7 = s1.prop7.toLowerCase();
...
currentTimeZoneOffsetInHours = currentTimeZoneOffsetInHours * -1;   //converts to correct offset as getTimezoneOffset returns f.e.â€-1â€ for GMT +1
s1.prop16 = s1.eVar16 = s1.getTimeParting('s', currentTimeZoneOffsetInHours);


/* Internal Search Query */
if (!s1.prop9) s1.prop9 = s1.getQueryParam('q');
if (s1.prop9) s1.prop9 = s1.prop9.toLowerCase();

/* External Campaigns  */
if (!s1.campaign) s1.campaign = s1.getQueryParam('cid');
if (s1.campaign) s1.campaign = s1.campaign.toLowerCase();

/* Campaign Stacking */
s1.prop14 = s1.eVar14 = s1.crossVisitParticipation(s1.campaign, 's_ev14', '30', '5', '>', '', 1);

/* Get Previous Page*/
s1.prop17 = s1.eVar17 = s1.getPreviousValue(s1.pageName, 'gpv_e17', '');

/* Build Content ID (prop15) */
s1.prop15 = s1.prop1 + ":" + s1.pageName;

/* Build PNRTimestamp (s.eVar50) */
s1.eVar50 = s1.purchaseID;

/*Track download links using linkDownloadFileType */
var link = s1.downloadLinkHandler();
if (link) {
	s1.prop18 = link;
	s1.eVar18 = link;
	s1.events = "event19"
	s1.linkTrackVars = "prop18,eVar18,events";
	s1.linkTrackEvents = "event19";
}
 
//Track outbound exit links using linkInternalFilters
var link = s1.exitLinkHandler();
if(link){
    s1.prop19=link;
	s1.eVar19=link;
    s1.linkTrackVars="prop19,eVar19";
}
...
/*
61.s1.events="event10"	Book Chauffeur Start		Set when s1.pageName="plan-and-book:book-chauffeur"
62.s1.events="event12"	Newsletter Signup Start		Set when s1.pageName="newsletter"
63.s1.events="event13"	Newsletter Signup Complete	Set when s1.pageName="newsletter:confirmation"
68.s1.events="event22"	Upgrade Start				Set when s1.pageName="eyey:upgrade_ancillary_page"
69.s1.events="event23"	Upgrade Complete			Set when s1.pageName="eyey:upgrade_confirmation_page"
*/
	if(s1.pageName.indexOf("plan-and-book:book-chauffeur") != -1){
		s1.events=s1.apl(s1.events,"event10",",",2);
	}
	if(s1.pageName.indexOf("newsletter") != -1){
		s1.events=s1.apl(s1.events,"event12",",",2);
	}
	if(s1.pageName.indexOf("newsletter:confirmation") != -1){
		s1.events=s1.apl(s1.events,"event13",",",2);
	}
	if(s1.pageName.indexOf("eyey:upgrade_ancillary_page") != -1){
		s1.events=s1.apl(s1.events,"event22",",",2);
	}
	if(s1.pageName.indexOf("eyey:upgrade_confirmation_page") != -1){
		s1.events=s1.apl(s1.events,"event23",",",2);
	}


/*
56.s1.events="event4"	Etihad Guest Register Start	Set when s1.pageName="etihadguest:join"
57.s1.events="event5"	Etihad Guest Register Complete	Set when s1.pageName="etihadguest:join:confirmation"
*/
	if(s1.pageName.indexOf("etihadguest:join") != -1){
		if(s1.pageName.indexOf("etihadguest:join:confirmation") != -1) {
	   		s1.events=s1.apl(s1.events,"event5",",",2);
	   	}
	   	else {
	   		s1.events=s1.apl(s1.events,"event4",",",2);
	   	}
	}
...
/*
51.s1.events="scCheckout"	Checkout		Set when s1.pageName="eyey:purchase_page" OR s1.pageName="eym0:purchase_page"
52.s1.events="purchase"		Order			Set when s1.pageName="eyey:confirmation_page" OR s1.pageName="eym0:confirmation_page"
53.s1.events="event1"		Flight Search		Set when s1.pageName="eyey:air_select_page" OR s1.pageName="eym0:air_select_page"
54.s1.events="event2"		Select Flight		Set when s1.pageName="eyey:passengers_page" OR s1.pageName="eym0:passengers_page"
58.s1.events="event6"		Exchange Start		Set when s1.pageName="eyey:exchange_air_search_page" OR s1.pageName="eym0:exchange_air_search_page"
59.s1.events="event7"		Exchange Complete	Set when s1.pageName="eyey:exchange_confirmation_page" OR s1.pageName="eym0:exchange_confirmation_page"
60.s1.events="event8"		Check-in Start		Set when s1.pageName="eyey:check_in_start_page" OR "eym0:check_in_start_page"
65.s1.events="event18"		MMB Sale Amount		Set when s1.pageName="eyey:exchange_confirmation_page" OR s1.pageName="eym0:exchange_confirmation_page"	?????
*/
		if(s1.pageName.indexOf("eyey:purchase_page") != -1 || s1.pageName.indexOf("eym0:purchase_page") != -1){
	   	s.events="scCheckout"
	}
	if(s1.pageName.indexOf("eyey:confirmation_page") != -1 || s1.pageName.indexOf("eym0:confirmation_page") != -1){
	   	s1.events="purchase"
	}
	if(s1.pageName.indexOf("eyey:air_select_page") != -1 || s1.pageName.indexOf("eym0:air_select_page") != -1){
	   	s.events="event1"
	}
	if(s1.pageName.indexOf("eyey:passengers_page") != -1 || s1.pageName.indexOf("eym0:passengers_page") != -1){
	   	s.events="event2"
	}
	if(s1.pageName.indexOf("eyey:exchange_air_search_page") != -1 || s1.pageName.indexOf("eym0:exchange_air_search_page") != -1){
	   	s1.events="event6"
	}
	if(s1.pageName.indexOf("eyey:exchange_confirmation_page") != -1 || s1.pageName.indexOf("eym0:exchange_confirmation_page") != -1){
	   	s1.events="event7,event18"
	   	
	}
	if(s1.pageName.indexOf("eyey:check_in_start_page") != -1 || s1.pageName.indexOf("eym0:check_in_start_page") != -1){
	   	s1.events="event8"
	}
...
/*
55.s1.events="event3"	Sabre Error	Set when s1.prop10 is populated.
*/
	if(s1.prop10) {
...
/*
64.s1.events="event17"	Guest Miles Spent	Set when s1.pageName="eyey:confirmation_page" AND s1.prop7="redemption"
*/
	if(s1.pageName && s1.prop7) {
		if(s1.pageName.indexOf("eyey:confirmation_page") != -1 && s1.prop7.indexOf("redemption") != -1) {
			s1.events=s1.apl(s1.events,"event17",",",2)
		}
	}


/*
67.s1.events="event21"	Affiliate Outbound Click	Set when s1.prop19 contains hertz or kuoni
*/
	if(s1.prop19) {
		if(s1.prop19.indexOf("hertz") != -1 || s1.prop19.indexOf("kuoni") != -1) {
			s1.events=s1.apl(s1.events,"event21",",",2)
		}
	}
...
if (s1.prop1 && !s1.eVar1) s1.prop1 = s1.eVar1 = s1.prop1.toLowerCase();
if (s1.prop2 && !s1.eVar2) s1.prop2 = s1.eVar2 = s1.prop2.toLowerCase();
if (s1.prop3 && !s1.eVar3) s1.prop3 = s1.eVar3 = s1.prop3.toLowerCase();
if (s1.prop4 && !s1.eVar4) s1.prop4 = s1.eVar4 = s1.prop4.toLowerCase();
if (s1.prop5 && !s1.eVar5) s1.prop5 = s1.eVar5 = s1.prop5.toLowerCase();
if (s1.prop6 && !s1.eVar6) s1.prop6 = s1.eVar6 = s1.prop6.toLowerCase();
if (s1.prop7 && !s1.eVar7) s1.eVar7 = s1.prop7.toLowerCase();
if (s1.prop8 && !s1.eVar8) s1.prop8 = s1.eVar8 = s1.prop8.toLowerCase();
if (s1.prop9 && !s1.eVar9) s1.prop9 = s1.eVar9 = s1.prop9.toLowerCase();
if (s1.prop10 && !s1.eVar10) s1.prop10 = s1.eVar10 = s1.prop10.toLowerCase();
if (s1.prop11 && !s1.eVar11) s1.prop11 = s1.eVar11 = s1.prop11.toLowerCase();
if (s1.prop12 && !s1.eVar12) s1.prop12 = s1.eVar12 = s1.prop12.toLowerCase();
if (s1.prop14 && !s1.eVar14) s1.prop14 = s1.eVar14 = s1.prop14.toLowerCase();
if (s1.prop15 && !s1.eVar15) s1.prop15 = s1.eVar15 = s1.prop15.toLowerCase();
if (s1.prop17 && !s1.eVar17) s1.prop17 = s1.eVar17 = s1.prop17.toLowerCase();
if (s1.prop18) s1.prop18 = s1.prop18.toLowerCase();
if (s1.eVar18) s1.eVar18 = s1.eVar18.toLowerCase();
if (s1.prop19) s1.prop19 = s1.prop19.toLowerCase();
if (s1.eVar19) s1.eVar19 = s1.eVar19.toLowerCase();
if (s1.prop24 && !s1.eVar24) s1.prop24 = s1.eVar24 = s1.prop24.toLowerCase();
...
*/
s1.inList = new Function("v", "l", "d", ""
+ "var s=this,ar=Array(),i=0,d=(d)?d:',';if(typeof(l)=='string'){if(s1."
+ "split)ar=s1.split(l,d);else if(l.split)ar=l.split(d);else return-1}e"
 ...
 */
s1.getTimeParting=new Function("h","z",""
+"var s=this,od;od=new Date('1/1/2000');if(od.getDay()!=6||od.getMont"
+"h()!=0){return'Data Not Available';}else{var H,M,D,U,ds,de,tm,da=['"
+"Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturda"
+"y'],d=new Date();z=z?z:0;z=parseFloat(z);if(s1._tpDST){var dso=s1._tp"
...
*/
s1.getQueryParam = new Function("p", "d", "u", ""
+ "var s=this,v='',i,t;d=d?d:'';u=u?u:(s1.pageURL?s1.pageURL:s1.wd.locati"
+ "on);if(u=='f')u=s1.gtfs().location;while(p){i=p.indexOf(',');i=i<0?p"
+ ".length:i;t=s1.p_gpv(p.substring(0,i),u+'');if(t){t=t.indexOf('#')>-"
+ "1?t.substring(0,t.indexOf('#')):t;}if(t)v+=v?d+t:t;p=p.substring(i="
+ "=p.length?i:i+1)}return v");
s1.p_gpv = new Function("k", "u", ""
+ "var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+ "=s1.pt(q,'&','p_gvf',k)}return v");
s1.p_gvf = new Function("t", "k", ""
+ "if(t){var s=this,i=t.indexOf('='),p=i<0?t:t.substring(0,i),v=i<0?'T"
+ "rue':t.substring(i+1);if(p.toLowerCase()==k.toLowerCase())return s1."
+ "epa(v)}return ''");

/*
* Utility Function: split v1.5 (JS 1.0 compatible)
*/
s1.split = new Function("l", "d", ""
...
*/
s1.downloadLinkHandler=new Function("p",""
+"var s=this,h=s1.p_gh(),n='linkDownloadFileTypes',i,t;if(!h||(s1.linkT"
+"ype&&(h||s1.linkName)))return '';i=h.indexOf('?');t=s[n];s[n]=p?p:t;"
+"if(s1.lt(h)=='d')s1.linkType='d';else h='';s[n]=t;return h;");

/*
 * Plugin: exitLinkHandler 0.5 - identify and report exit links
 */
s1.exitLinkHandler=new Function("p",""
+"var s=this,h=s1.p_gh(),n='linkInternalFilters',i,t;if(!h||(s1.linkTyp"
+"e&&(h||s1.linkName)))return '';i=h.indexOf('?');t=s[n];s[n]=p?p:t;h="
+"s1.linkLeaveQueryString||i<0?h:h.substring(0,i);if(s1.lt(h)=='e')s1.li"
...
*/
s1.p_gh = new Function(""
+ "var s=this;if(!s1.eo&&!s1.lnk)return '';var o=s1.eo?s1.eo:s1.lnk,y=s1.ot("
+ "o),n=s1.oid(o),x=o.s_oidt;if(s1.eo&&o==s1.eo){while(o&&!n&&y!='BODY'){"
+ "o=o.parentElement?o.parentElement:o.parentNode;if(!o)return '';y=s1."
+ "ot(o);n=s1.oid(o);x=o.s_oidt}}return o.href?o.href:'';");

/*
* Plugin Utility: apl v1.1
*/
s1.apl = new Function("l", "v", "d", "u", ""
+ "var s=this,m=0;if(!l)l='';if(u){var i,n,a=s1.split(l,d);for(i=0;i<a."
 ...
 */
s1.crossVisitParticipation=new Function("v","cn","ex","ct","dl","ev","dv",""
+"var s=this,ce;if(typeof(dv)==='undefined')dv=0;if(s1.events&&ev){var"
+" ay=s1.split(ev,',');var ea=s1.split(s1.events,',');for(var u=0;u<ay.l"
+"ength;u++){for(var x=0;x<ea.length;x++){if(ay[u]==ea[x]){ce=1;}}}}i"
+"f(!v||v==''){if(ce){s1.c_w(cn,'');return'';}else return'';}v=escape("
+"v);var arry=new Array(),a=new Array(),c=s1.c_r(cn),g=0,h=new Array()"
+";if(c&&c!=''){arry=s1.split(c,'],[');for(q=0;q<arry.length;q++){z=ar"
+"ry[q];z=s1.repl(z,'[','');z=s1.repl(z,']','');z=s1.repl(z,\"'\",'');arry"
+"[q]=s1.split(z,',')}}var e=new Date();e.setFullYear(e.getFullYear()+"
+"5);if(dv==0&&arry.length>0&&arry[arry.length-1][0]==v)arry[arry.len"
+"gth-1]=[v,new Date().getTime()];else arry[arry.length]=[v,new Date("
+").getTime()];var start=arry.length-ct<0?0:arry.length-ct;var td=new"
+" Date();for(var x=start;x<arry.length;x++){var diff=Math.round((td."
+"getTime()-arry[x][1])/86400000);if(diff<ex){h[g]=unescape(arry[x][0"
+"]);a[g]=[arry[x][0],arry[x][1]];g++;}}var data=s1.join(a,{delim:',',"
+"front:'[',back:']',wrap:\"'\"});s1.c_w(cn,data,e);var r=s1.join(h,{deli"
+"m:dl});if(ce)s1.c_w(cn,'');return r;");

/*
* s1.join: 1.0 - Joins an array into a string
*/
s1.join = new Function("v", "p", ""
+ "var s = this;var f,b,d,w;if(p){f=p.front?p.front:'';b=p.back?p.back"
+ ":'';d=p.delim?p.delim:'';w=p.wrap?p.wrap:'';}var str='';for(var x=0"
+ ";x<v.length;x++){if(typeof(v[x])=='object' )str+=s1.join( v[x],p);el"
...
*/
s1.getPreviousValue = new Function("v", "c", "el", ""

+ "var s=this,t=new Date,i,j,r='';t.setTime(t.getTime()+1800000);if(el"
+ "){if(s1.events){i=s1.split(el,',');j=s1.split(s1.events,',');for(x in i"
+ "){for(y in j){if(i[x]==j[y]){if(s1.c_r(c)) r=s1.c_r(c);v?s1.c_w(c,v,t)"
+ ":s1.c_w(c,'no value',t);return r}}}}}else{if(s1.c_r(c)) r=s1.c_r(c);v?"
+ "s1.c_w(c,v,t):s1.c_w(c,'no value',t);return r}");

/*
* Plugin Utility: Replace v1.0
*/
s1.repl = new Function("x", "o", "n", ""
...
s1.visitorNamespace = "etihadairways"
s1.trackingServer = "metrics.etihad.com"
s1.trackingServerSecure = "smetrics.etihad.com"
